...
int main()
{
    ...
    {
        printf("%.4f, %.4f and %.4f Is Not!",a[0],a[1],a[2]); // ????????????????????
        return 0;
    }
    else
    {   int i;
        ...
        if(max >= min + mid)
        {
            printf("%.4f, %.4f and %.4f are NOT sides of triangle.",a[0],a[1],a[2]); // ????????????????????
            return 0;
        }
        //printf("%d\n",(max > sqrt(pow(mid,2)+pow(min,2))));
        if((max-0.000001 < sqrt(pow(mid,2)+pow(min,2))) && (max+0.000001 > sqrt(pow(mid,2)+pow(min,2))))
        {
            printf("%.4f, %.4f and %.4f are sides of RIGHT triangle.",a[0],a[1],a[2]); // ?????????????????
            return 0;
        }
